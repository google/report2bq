...
def main(unused_argv):
  fetcher = fetcher_factory.create_fetcher(Type(
      FLAGS.product), email=FLAGS.email, project=FLAGS.project, profile=FLAGS.profile)
  ...
  if fetcher.report_type == Type.CM:
    ...
    if FLAGS.backup:
      report = fetcher.get_report_definition(
          profile_id=FLAGS.profile, report_id=FLAGS.report_id,)
      with open(f'config_files/{FLAGS.report_id}.json', 'w') as report_file:
